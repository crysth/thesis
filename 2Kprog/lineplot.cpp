...
--------------------------------------------------------------------*/
Lineplot::~Lineplot(){
	_f1.close(); _f2.close(); _f3.close(); _f4.close(); _f5.close(); //_f6.close(); _f7.close(); _f8.close(); _f9.close();
};
/*--------------------------------------------------------------------
 Functions
--------------------------------------------------------------------*/
...
/*------------------------------------------------------------------
//stampalitutti e poi faremo la media!
void Lineplot::ldeltal(Packagegraph *pac){
	if (check==0) cerr<<"Lineplot:: path non dichiarato!"<<endl;
	else {
		if (l_check==0) {
			system(("mkdir -p "+_path+"/orderparameter/").c_str());
			string dot=".dat";
			_f4.open(_path+"/orderparameter/"+_language+".ldeltal."+dot,ios::out);
			_f4<<"#Numero_nodi\tmean_depth\n";
			l_check=1;
		}
		for(size_t k=0; k<pac->N(); k++) {
			for(size_t j=1; j<pac->depth(k); j++){
				delta[j]+=(double)pac->RNnodes(k,j)/pac->RNnodes(k,j+1);
				contatore[j]++;
			}
		}
		_f4 << j << "\t" << (double)pac->RNnodes(k,j)/pac->RNnodes(k,j+1) << "\t" << pac->Nnodes(k) << endl;
	}
};
/*------------------------------------------------------------------*/
