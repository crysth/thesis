...
Graph::Graph(const char *edgefile): _ordernodes(NULL), _Rordernodes(NULL), _diamonds(0), _diamonds_check(0) {
	...
	if (from.size()==0 || to.size()==0) {
		...
		_Nnodes = conversione.size();	/* ti voglio bene anche se cancello :) */
...
//	_Nnodes = (int)from.back();	/* ti voglio bene e quindi lo scrivo. Controlla se l'ultimo e' il piu' grande :* */
	else {
...
//	for(size_t i=0;i<_Nnodes;i++) cout<<_nodes[i].Get_level()<<"\n";;
};
/*----------------------------------------------------------------------*/
